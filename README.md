...
This README is for developers. Architecture decisions live in
[`docs/adr/`](#documentation-map); per-crate guides in
[`docs/modules/`](#documentation-map).
...
cd ../core && cargo test --workspace      # 110 tests
```

## Features

The app (sidebar → view) covers:

- **Dashboard** — Mac health ring, live CPU/memory line graphs, storage,
  battery (cycles/health), thermal pressure, free-space trend.
- **Smart Scan / Junk / Large & Old** — streaming scan with a live radar
  loader and per-category chips; review every item before anything is trashed.
- **Duplicates** — whole-home blake3 funnel; pick which copies to remove.
- **Uninstaller** — remove apps and trace their leftover support files;
  disk-wide orphan scan for already-removed apps.
- **Developer / CLI** — Homebrew analysis + safe cleanup: `brew cleanup` /
...
│       ├── tabibu-brew/        # Homebrew analysis + safe cleanup (delegates to brew)
...
│       ├── icons/              # app icon set (generated by `tauri icon`)
│       └── src/                # main.rs, commands.rs, system.rs, tray.rs
├── scripts/                    # bench-gate, uninstall-tabibu
...
platform. Full rationale: **ADR-0003**. The
...
```sh
cd core
cargo test --workspace            # 110 tests: unit, integration, property, golden-image
...
A designed, interactive overview lives at [`docs/index.html`](docs/index.html)
(open it in a browser). Source docs:
...
1. **For a new scanner:** implement `Scanner`, register in your crate's
...
2. **For any mutating change:** design the undo first; add a golden-image test
   before implementing.
3. **Before pushing:** the gates above must be green, and the relevant `docs/`
   updated in the same change (stale docs are bugs).
